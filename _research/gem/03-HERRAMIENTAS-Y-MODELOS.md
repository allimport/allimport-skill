...
- Claude arranca una sesión nueva y necesita entender el estado del proyecto (en vez de
  releer 10+ archivos de `_research/` o `docs/` uno por uno).
...
- Es un cambio puntual en 1-2 archivos conocidos (Claude los lee directo, más rápido que
  consultar el grafo).
- Es código de `web/` (Graphify se usa hoy sobre `_research/`, no sobre la app).

**Mantenimiento (decirle a Claude que lo corra cuando corresponda):**
Después de agregar o cambiar documentos importantes en `_research/`, `docs/` o
`contenido/`, Claude debe re-indexar:
```
...
`skills/` y `web/` (probado: 16.025 nodos vs 197 útiles solo en `_research/`). Indexar por
...
| Clip de producto animado | `remotion-video-creation` (proyecto real en `video/`) |
...
| Necesito... | Skill exacta |
|---|---|
| Ajustar el comportamiento/reglas del agente | editar `contenido/AGENTE-WHATSAPP.md` directo (no hay skill; es un doc de spec) |
| Que suene como el dueño | `contenido/VOZ-AGUS.md` (fuente de verdad del tono) |
