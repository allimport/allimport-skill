...
- `allimport/docs/` — núcleo documental (SSOT) + `MAPA-CONOCIMIENTO.md` (hub Obsidian/Graphify).
- `allimport/contenido/` — CALENDARIO-SEMANAL, GANCHOS-Y-GUIONES, DESIGN, VOZ-AGUS, AGENTE-WHATSAPP, EJEMPLOS-RESPUESTAS.
- `allimport/historias/` — editor de fotos → historias IG. `allimport/video/` — proyecto Remotion.
- `allimport/proveedores/` — analizador de chats + template CRM.
- `allimport/_research/` — plan, objetivos, investigación (este paquete).
- Verificación estándar: `cd web && npx tsc --noEmit && npm run build`.

## FASE A · Contenido (O1, foco #1, hacer primero)
| # | Paso | Estado | Quién |
|---|---|---|---|
| A1 | Investigar viralidad del nicho (qué funciona + por qué) | ✅ `allimport/contenido/HALLAZGOS-VIRALES.md` | — |
| A2 | 3 guiones de reel basados en esa investigación | ✅ `allimport/contenido/GANCHOS-Y-GUIONES.md` | — |
...
| E9 | Flujos de trabajo separados por módulo (3 chats: contenido/visual/agente+CRM) | ✅ `allimport/docs/FLUJOS-DE-TRABAJO.md` | — |

**E2 detalle (recién completado):** Obsidian instalado apuntando a la raíz del repo +
Graphify (`graphifyy` vía pipx) instalado, indexando `allimport/_research/` con extracción semántica
por Gemini (API key propia), volcado dentro de la bóveda de Obsidian (`--obsidian`). Los
dos leen los mismos archivos, sin sync. Ver `allimport/docs/OBSIDIAN-GRAPHIFY.md`.
