...
### Estructura nueva
- Carpetas creadas con su `README.md`: `allimport/contenido/` · `allimport/video/` · `allimport/historias/` · `allimport/proveedores/`.

### Conocimiento generado (Claude, sin inventar datos)
- `allimport/contenido/CALENDARIO-SEMANAL.md` — grilla fija: 3 reels/semana (lun/mié/vie) + historias diarias (4-10) + bloque fijo diario.
- `allimport/contenido/GANCHOS-Y-GUIONES.md` — 10 ganchos con estructura hook→problema→solución→prueba social→CTA, tono argentino, cliente 18-28 que quiere emprender.
- `allimport/contenido/DESIGN.md` — branding: colores (`#0a0f1a`/`#00d4d4` + paleta), tipografía, tono de voz, frases que SÍ / que NO.
- `allimport/proveedores/base-datos-clientes-template.csv` — plantilla (nombre, fecha, producto, monto, pago, envío/retiro, teléfono, etiqueta compró/potencial/curioso).
- `allimport/docs/README.md` — actualizado: enlaza plan definitivo, análisis de videos/fotos, pack del Gem y las carpetas nuevas.
- `CLAUDE.md` — registra las carpetas nuevas, el foco en redes y los agentes instalados.
- `allimport/docs/TERCEROS-PENDIENTES.md` — terceros que quedaron sin instalar y por qué.

### Herramientas ya hechas, movidas al repo
- `allimport/historias/editar_fotos.py` + `CORRER_EDITAR.bat` — editor de fotos → historias.
- `allimport/proveedores/analizar_completo.py` + `CORRER_COMPLETO.bat` + `LEEME.txt` — analizador de chats.
- `allimport/video/` — proyecto **Remotion** completo (`src/`, `public/producto.jpg`, config). Sin `node_modules`/`out` (se regeneran; agregados a `.gitignore`).
...
3. 📱💻 **Grabar el primer reel** con un guion de `allimport/contenido/GANCHOS-Y-GUIONES.md` y editarlo en CapCut. (Los subtítulos/contadores te los genero yo.)
4. 📱 **Historias diarias** siguiendo `allimport/contenido/CALENDARIO-SEMANAL.md`.
5. 💻 **Correr el editor de fotos** (`allimport/historias/`) con tus fotos reales → subís las historias 📱.

### Ventas y datos (FASE 2)
6. 📱 **Etiquetar WhatsApp** (compró/potencial/curioso) y cargar `allimport/proveedores/base-datos-clientes-template.csv`.
7. 💻 **Correr el analizador de proveedores** (`allimport/proveedores/`) cuando quieras la lista de precios del mes.
